...
const DeleteConfirmModal: React.FC<{ 
  ...
  onConfirm: () => void;
  alerteType: string;
}> = ({ isOpen, onClose, onConfirm, alerteType }) => {
  if (!isOpen) return null;
      ...
      >
        ...
        <div className="p-6 text-center">
          ...
          <p className="text-gray-600 mb-4">
            Êtes-vous sûr de vouloir supprimer l'alerte "<strong>{alerteType}</strong>" ?
          </p>
...
interface AlerteItemProps {
  ...
  onEdit: (alerte: Alerte) => void;
  onTerminer: (id: number) => void;
}

const AlerteItem: React.FC<AlerteItemProps> = ({ alerte, goToMap, onPhotoClick, onDelete, onEdit, onTerminer }) => {
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg hover:shadow-xl transition duration-300 border border-gray-100 flex flex-col">
      ...
      {photoUrl && (
        <div className="mb-3">
          <button onClick={() => onPhotoClick(photoUrl)} className="w-full">
            <img
              src={photoUrl}
              alt="Photo"
              className="w-full h-48 md:w-24 md:h-24 object-cover rounded-lg border border-gray-200 shadow-sm cursor-pointer hover:opacity-80"
      ...
      {/* MODIFICATION : Ajout de l'heure et minute avec icône Clock */}
      ...
      <div className="flex justify-end mt-auto">
        <button
          ...
          className={`flex items-center justify-center font-medium py-2 px-4 rounded-full text-sm whitespace-nowrap transition shadow-md ${
            hasLocation ? 'bg-indigo-500 text-white hover:bg-indigo-600' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
          }`}
        >
          {hasLocation ? (
            <>
...
const ListeAlerteComponent: React.FC<ListeAlerteProps> = ({ goToMapWithAlert }) => {
  ...
  // État pour le filtre de type d'alerte
  ...
  const [deleteConfirmModal, setDeleteConfirmModal] = useState({ isOpen: false, alerte: null as Alerte | null });
  ...
  const handleEditAlert = (alerte: Alerte) => {
  ...
  const handleSaveAlert = (updated: Alerte) => {
    setAlerts(prev => prev.map(a => (a.id === updated.id ? updated : a)));
    handleCloseEditModal();
    showSuccessMessage("Alerte modifiée avec succès");
  };
  ...
  const openDeleteConfirm = (alerte: Alerte) => {
    setDeleteConfirmModal({ isOpen: true, alerte });
  };

  const confirmDelete = () => {
    if (deleteConfirmModal.alerte) {
      handleDeleteAlert(deleteConfirmModal.alerte.id);
      setDeleteConfirmModal({ isOpen: false, alerte: null });
    }
  };

  if (isLoading) return <div className="flex justify-center items-center h-64"><Loader2 className="w-8 h-8 animate-spin text-indigo-600" /><p className="ml-3">Chargement...</p></div>;
  ...
  return (
    <div className="p-4 md:p-8 bg-gradient-to-br from-red-50 to-green-50 min-h-screen relative">
        ...
        <div className="space-y-4 max-w-4xl mx-auto">
          {filteredAlerts.map((alerte) => (
            <AlerteItem
              ...
              onDelete={() => openDeleteConfirm(alerte)}
      ...
      <DeleteConfirmModal
        ...
        onClose={() => setDeleteConfirmModal({ isOpen: false, alerte: null })}
        onConfirm={confirmDelete}
        alerteType={deleteConfirmModal.alerte?.type_alerte || ""}
      />
    </div>
  );
};
