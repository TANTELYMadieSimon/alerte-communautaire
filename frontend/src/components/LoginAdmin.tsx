...
const djangoAdminAPI = {
  ...
  async createAdmin(adminData: any) {
    ...
    formData.append('password_confirm', adminData.passwordConfirm);
    formData.append('sms_phone', adminData.phone || '');

    if (adminData.profilePhoto && adminData.profilePhoto.startsWith('data:')) {
      const response = await fetch(adminData.profilePhoto);
      const blob = await response.blob();
      formData.append('profile_photo', blob, 'profile.jpg');
    }

    const response = await fetch(`${this.baseURL}/api/admins/create/`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });
  ...
  async logoutAdmin() {
    const response = await fetch(`${this.baseURL}/api/admins/logout/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      throw new Error(data.message || 'Erreur de déconnexion admin');
    }
  ...
  async getAdminProfile() {
    const response = await fetch(`${this.baseURL}/api/admins/profile/`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      throw new Error(data.message || 'Erreur lors de la récupération du profil admin');
    }
...
interface LoginAdminProps {
  onLogin: (role: "admin", adminData?: any) => void;
...
export const LoginAdmin: React.FC<LoginAdminProps> = ({ onLogin, onMessage }) => {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await djangoAdminAPI.createAdmin(adminData);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await djangoAdminAPI.loginAdmin(credentials);
      
      if (result.success) {
        const adminWithPhoto = {
          ...result.admin,
          profile_photo_url: result.admin.profile_photo_url || null
        };

        localStorage.setItem('currentAdmin', JSON.stringify(adminWithPhoto));
        onLogin("admin", adminWithPhoto);
        onMessage(result.message, "success");
      } else {
          ...
          <div className="w-full max-w-md mx-auto space-y-8">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 mt-2">
                {isCreatingAccount ? "Créer un compte Admin" : "Connexion Administrateur"}
              </p>
            </div>
              ...
              <div className="space-y-4">
                ...
                {isCreatingAccount && (
                  <div className="flex flex-col items-center mb-4">
                    <div className="relative">
                      <div className="w-20 h-20 rounded-full border-4 border-indigo-200 overflow-hidden bg-gray-100 flex items-center justify-center">
                        {profilePhoto ? (
                      ...
                      <button
                        ...
                        className="absolute bottom-0 right-0 bg-indigo-600 text-white p-1.5 rounded-full shadow-lg hover:bg-indigo-700 transition duration-200"
                      >
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Votre nom d'administrateur"
                      value={username}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      minLength={6}
                      className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-150 pr-10"
                    />
                    <button
                      type="button"
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className={`w-full text-white py-3 rounded-lg font-bold uppercase tracking-wider transition duration-300 shadow-lg transform hover:scale-[1.01] disabled:opacity-50 disabled:cursor-not-allowed ${
                    isCreatingAccount 
                ...
                >
                  {isLoading ? "Chargement..." : 
                   isCreatingAccount ? "Créer le compte admin" : "Se connecter"}
                </button>
              </div>
            </form>
            ...
            <div className="text-center">
              ...
              >
                {isCreatingAccount ? "Déjà un compte ? Se connecter" : "Créer un compte administrateur"}
              </button>
            </div>
          ...
          <div className="relative z-10 flex flex-col justify-center h-full px-6 lg:px-12 text-white">
            <div className="text-center">
              <h1 className="text-3xl lg:text-5xl font-bold mb-4 lg:mb-6 text-green-400">
                <TextType 
                  text={["Administration", "Gestion", "Sécurité"]}
                  typingSpeed={75}
                  pauseDuration={2500}
                  showCursor={true}
                  cursorCharacter="|"
              ...
              <h3 className="text-lg lg:text-2xl font-bold text-white">
                <TextType 
                  text={[
                    "Gestion des alertes communautaires",
                    "Supervisez votre communauté", 
                    "Système d'alerte sécurisé"
                  ]}
                  typingSpeed={50}
                  pauseDuration={2000}
                  showCursor={true}
